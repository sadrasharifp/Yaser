...
class LocalAttention(nn.Module):
    def __init__(self,
        ...
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
        self.window_size = window_size  # Define local window size
        self.scale = self.head_dim ** -0.5
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.q_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
    ...
    def forward(self, x: torch.Tensor):
        ...
        attn = torch.matmul(q, k.transpose(-2, -1))  # Compute attention scores

        # Apply local windowing: use a sliding window to mask irrelevant attention scores
        mask = torch.zeros_like(attn)  # Masking irrelevant attention scores
        for i in range(N):
            start = max(0, i - self.window_size // 2)
            end = min(N, i + self.window_size // 2 + 1)
            mask[:, :, i, start:end] = 1.0

        # Now apply the mask to the attention scores
        mask = (mask == 0).float() * -1e9  # Masking out the irrelevant areas by setting them to -inf

        attn = attn + mask  # Apply the mask to the attention scores (i.e., -inf to masked areas)
        attn = attn.softmax(dim=-1)  # Apply softmax to normalize the attention weights

        # Apply the attention and projection
...
class LocalAttention_o(nn.Module):
    def __init__(self,
                 dim: int,
                 num_heads: int = 8,
                 window_size: int = 11,  # 初始窗口大小（可作为参考）
                 qkv_bias: bool = False,
                 qk_norm: bool = False,
                 proj_bias: bool = True,
                 attn_drop: float = 0.,
        ...
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
        # 这里我们不再直接使用固定的window_size，而是定义一个可学习参数 window_param
        # 我们将其初始化为 window_size，并且在前向时使用 clamped 版本，确保其在 [3, 79] 范围内
        self.window_param = nn.Parameter(torch.tensor(float(window_size)))
        self.scale = self.head_dim ** -0.5
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.q_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
    ...
    def forward(self, x: torch.Tensor):
        ...
        attn = torch.matmul(q, k.transpose(-2, -1))  # shape: [B, num_heads, N, N]

        # 得到一个距离矩阵，形状为 [N, N]
        positions = torch.arange(N, device=x.device).unsqueeze(0)
        distance_matrix = torch.abs(positions - positions.transpose(0, 1)).float()  # [N, N]

        # 用 window_param 计算衰减 bias：
        # 这里我们使用高斯衰减公式: - (d^2) / (2 * sigma^2)，sigma 由 window_param 决定
        # 为了保证 window_param 在合理范围，我们可以 clamp 它
        sigma = torch.clamp(self.window_param, 3, 40)
        bias = - (distance_matrix ** 2) / (2 * sigma ** 2)  # [N, N]

        # 将 bias 扩展到 [B, num_heads, N, N] 并加到 attn 上
        bias = bias.unsqueeze(0).unsqueeze(0)  # [1,1,N,N]
        attn = attn + bias

        # 接下来 softmax 与 dropout
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class LocalAttention_oi(nn.Module):
    def __init__(self,
                 dim: int,
                 num_heads: int = 8,
                 max_len: int = 129,      # 序列（patch）的最大长度
                 init_sigma: float = 11.,  # σᵢ 的初始值
                 qkv_bias: bool = False,
                 qk_norm: bool = False,
                 proj_bias: bool = True,
                 attn_drop: float = 0.,
        ...
        # 可训练的 σ 向量，长度为 max_len
        # 每个 i 都有自己的 window_params[i]
        self.window_params = nn.Parameter(
            torch.ones(max_len) * float(init_sigma)
        )
    ...
    def forward(self, x: torch.Tensor):
        B, N, C = x.shape  # B, num_patches, dim
        assert N <= self.window_params.shape[0], \
            f"Input length N={N} exceeds max_len={self.window_params.shape[0]}"
        ...
        # 4) 每个位置自己的 σᵢ，并 clamp 到 [3, 40]
        sigmas = torch.clamp(self.window_params[:N], min=3.0, max=40.0)  # [N]
        sigmas = sigmas.unsqueeze(1)  # [N, 1]

        # 5) 构造 bias：−d²/(2 σᵢ²)，注意 σᵢ 对应每行 i
        bias = - (distance_matrix ** 2) / (2 * sigmas ** 2)  # [N, N]
        bias = bias.unsqueeze(0).unsqueeze(0)               # [1, 1, N, N]

        # 6) 加 bias → softmax → dropout
...
class Block_la_oi(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = LocalAttention_oi(
            dim,
            num_heads=num_heads,
            init_sigma = window_size,
            qkv_bias=qkv_bias,
            qk_norm=qk_norm,
            proj_bias=proj_bias,
            attn_drop=attn_drop,
